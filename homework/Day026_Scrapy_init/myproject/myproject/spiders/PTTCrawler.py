...
class PttcrawlerSpider(scrapy.Spider):
    ...
    #start_urls = ['https://www.ptt.cc/bbs/NBA/M.1586016394.A.D61.html']
    start_urls = ['https://www.ptt.cc/bbs/NBA/M.1586530963.A.0B2.html']
    cookies = {'over18': '1'}
    def start_requests(self):
        for url in self.start_urls:
            yield scrapy.Request(url=url, callback=self.parse, cookies=self.cookies)

    def saveSoup(self, text, filename):
        ff = open(filename, "w")
        ff.write(text)
        ff.close()
    ...
    def parse(self, response):
        ...
        self.saveSoup(soup.text, "day026_data.txt")
        # 取得文章內容主體
